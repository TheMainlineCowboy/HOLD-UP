...
object SubscriptionDraftParser {
    ...
    private val datePattern = Regex(
        "(?:cancel(?:lation)?(?: by| before)?|renews?(?: on)?|charge(?:d)?(?: on)?|effective(?: on)?)?\\s*((?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\\s+\\d{1,2}(?:,\\s*\\d{4})?|\\d{1,2}/\\d{1,2}(?:/\\d{2,4})?)",
        RegexOption.IGNORE_CASE
    )
    ...
    fun parse(rawText: String): SubscriptionDraft {
        ...
        return SubscriptionDraft(
            ...
            cadence = cadence,
            nextChargeOrDeadline = datePattern.find(text)?.groupValues?.getOrNull(1)?.trim(),
